...
import { Accordion, Button, Flex, Skeleton, Stack, Title } from "@mantine/core";
    ...
    return (
        <Stack gap='xs'>
            <Flex gap='md' p='md' ml='md'>
                <SearchableSelect key='search-origin'
                    label="Origin"
                    items={locationOptions}
                    defaultItem={null}
                    onChange={handleSearchInputChanged('origin')} />
                <SearchableSelect key='search-dest'
                    label="Destination"
                    items={locationOptions}
                    defaultItem={null}
                    onChange={handleSearchInputChanged('destination')} />
                <Button color="blue" mt='auto' disabled={originLocation === undefined || destinationLocation === undefined} onClick={handleSearchClick}>Search</Button> {/* Do I need to await the onclick? */}
            </Flex>
            {isLoading ?
                <> {Array(5).fill(0).map((_, index) => { return (<Skeleton height={30} key={'loading-sk-' + index} />); })} </>
                : <></>}
